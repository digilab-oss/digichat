...
FAIL=0            # a check found non-permissive code → the gate FAILs
SKIP_REQUIRED=0   # a check that CI can always run was skipped (check 2 — needs the built binary)
SKIP_OPTIONAL=0   # a defence-in-depth extra was skipped (check 4 — needs an image ref + syft)
# STRICT turns a SKIP_REQUIRED into a FAIL. CI sets it: the build runs before the gate, so a
# missing binary is a broken build, not an optional check. SKIP_OPTIONAL is never fatal.
STRICT="${LICENSE_GATE_STRICT:-0}"
...
#       same as Mattermost's own TE builds); any *sub*package there ships Source-Available code (FAIL).
...
if grep -qv '^github.com/mattermost/mattermost/server/v8/enterprise$' <<<"$mm" && [ -n "$mm" ]; then
  echo "  FAIL server links Source-Available subpackages:"; echo "$mm"; FAIL=1
...
#   the private EE repo) and `sourceavailable` (compiles the in-tree Source-Available packages). CI is
...
else echo "  SKIP: no shipped binary found (neither dist/ binary nor tarball) — run the build first"; SKIP_REQUIRED=1; fi
...
else echo "  SKIP: no image ref / syft. Dep licenses audited at baseline (all permissive bar mmctl-only isacikgoz/prompt=no-license, see README)."; SKIP_OPTIONAL=1; fi

echo
if [ "$FAIL" -ne 0 ]; then echo "### LICENSE GATE: FAIL"; exit 1; fi
if [ "$STRICT" = "1" ] && [ "$SKIP_REQUIRED" -ne 0 ]; then
  echo "### LICENSE GATE: FAIL (strict) — check 2 needs the built binary and it was missing."
  echo "    A build ran before this gate, so refuse to pass what we couldn't fully inspect."
  exit 1
fi
if [ "$SKIP_REQUIRED" -ne 0 ] || [ "$SKIP_OPTIONAL" -ne 0 ]; then
  echo "### LICENSE GATE: PASS (with skips — not a full gate; see SKIP lines)"
else
